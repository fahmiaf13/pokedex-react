...
export default function PokemonDetail() {
  ...
  const { data: pokemon, isLoading: isLoadingPokemons } = useQuery(["pokemons", id], async () => {
  ...
  // const { data: stats, isLoading: isLoadingStats } = useQuery(["stats", id], async () => {
  //   const response = await axios.get(config + "/stat/" + id);
  //   return response?.data;
  // });
          ...
          <Card className="w-1/2 min-h-[500px] p-10 rounded-xl relative my-[10rem]">
            <div className="flex">
              <div className="flex flex-col">
                <div className="text-6xl font-extrabold mt-9 mb-3">{title(pokemon.name)}</div>
            ...
            <div>{`${species?.flavor_text_entries[0].flavor_text} ${species?.flavor_text_entries[2].flavor_text} ${species?.flavor_text_entries[3].flavor_text}`}</div>
